...
BlockController.prototype.getPoolInfo = function(block) {
  ...
  var coinbaseAddress = block.transactions[0].outputs[0].script.toAddress(this.node.network);

/*    reward = parseInt(util.reverseHex(block.transactions[0].slice(102, 102 + 16)), 16);
    return {
	poolName: block.transactions[0].output[0].toString('hex'),
	url: ""
    };*/

  if (coinbaseAddress) {
    for(var k in this.poolStrings) {
       if (coinbaseAddress.toString('utf-8').match(k)) {
...
/*  var poolString = {
    poolName: coinbaseBuffer.toString('hex') + coinbaseBuffer.toString('utf-8'), // + coinbaseBuffer.toString('utf-8'),
    url: ""
  };
  return poolString;*/
  return {};
...
BlockController.prototype.getBlockReward = function(height) {
 var subsidy = new BN(100 * 1e8);
  ...
  if (height < (43200 / 2)) {
    subsidy /= 43200;
    subsidy = Math.floor(parseInt(subsidy.toString(10))); // Subsidy is calculated using int64_t in kotod
    subsidy *= height;
    return subsidy;
  } else if (height < 43200) {
    subsidy /= 43200;
    subsidy = Math.floor(parseInt(subsidy.toString(10)));
    subsidy *= (height+1);
    return subsidy;
  }

  var halvings = Math.floor((height - (43200/2)) / 1051200);
